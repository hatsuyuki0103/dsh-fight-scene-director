...
export function normalizeDescription(rawValue, followingLines = []) {
  ...
  if (value === '') return undefined
  // 任何以 `|` 或 `>` 开头的**非引号**标量都是块标量头。只匹配「合法拼写」会漏掉
  // `|--`、`|+2-`、`| -`、`|'` 这类畸形头，把字面量当描述发布出去——合法的 YAML
  // 解析器对它们一律报错，所以这里一律拒绝，让文件被跳过并诊断。
...
export function parseFrontmatter(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    // 键本身也可能是引号包裹的（`"userInvocable": false`）。不认这种写法会形成
    // 静默绕过：真实 YAML 会读出这个键，官方 provider 会拒绝它，而我们看不见它。
