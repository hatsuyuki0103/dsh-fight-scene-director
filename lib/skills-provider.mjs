 ...
 *
 * 顺序很关键：**先剥注释再判断块标量**。否则 `description: >- # note` 会因为尾部注释
 * 不匹配 `^[|>][+-]?\d*$` 而被当成普通文本，把字面量 `>-` 发布到技能目录里。
 *
 * 只接受：同一行内闭合的裸标量 / 单引号 / 双引号（双引号可跨物理行，因为换行已先归一化）。
 * YAML 块标量（`>-`、`|`、`|-`、`|2-`、`>2+` …）、只剩引号、空值一律返回 undefined。
...
export function normalizeDescription(rawValue, followingLines = []) {
  ...
  if (value === '') return undefined
  // 块标量指示符：`|` 或 `>`，可选 chomping 符号（+/-），可选缩进指示数（1-9），
  // 两者顺序都合法：`|-`、`|2`、`|2-`、`|-2`。
 ...
 * @param value - 原始标量文本。
 * @returns true / false / undefined（未提供或无法判定）。
 */
...
export function parseInvocationPolicy(raw) {
  ...
  const disableModelInvocation = frontmatterBoolean(raw['disable-model-invocation'])
  const userInvocable = frontmatterBoolean(raw['user-invocable'])
  return {
...
export function parseFrontmatter(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    meta[m[1]] = stripYamlComment(m[2]).trim()
